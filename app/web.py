...
def create_app(settings: Settings) -> FastAPI:
        ...
        try:
            ...
            industries = [
                ...
                for row in repository.connection.execute(
                    ...
                    FROM score_runs
                    WHERE mvp_run_id = ? AND status = 'SUCCEEDED'
                      AND json_extract(reason_json, '$.explicit_industry') IS NOT NULL
                    ORDER BY 1
                    """,
                    (selected_run,),
                ).fetchall()
    ...
    async def complete_review(request: Request, signal_id: str) -> RedirectResponse:
        ...
        try:
            ...
            run_id = _required(form, "run_id")
            workflow.present_score(run_id, signal_id, _required(form, "score_run_id"))
            workflow.complete_review(
                ...
                started_at=_required(form, "started_at"),
                completed_at=_required(form, "completed_at"),
                active_seconds=_integer(form, "active_seconds"),
    ...
    async def create_draft(request: Request, signal_id: str) -> RedirectResponse:
        ...
        try:
            ...
            Workflow(repository).create_draft(
                run_id=run_id, signal_id=signal_id, body=_required(form, "body")
            )
        except (KeyError, ValueError) as error:
            raise HTTPException(400, str(error)) from error
        finally:
    ...
    async def register_outreach(request: Request, signal_id: str) -> RedirectResponse:
        ...
        try:
            ...
            Workflow(repository).register_outreach(
                ...
                platform=_required(form, "platform"),
                subject_key=_required(form, "subject_key"),
    ...
    def followups(request: Request, run_id: str | None = None) -> HTMLResponse:
        ...
        return templates.TemplateResponse(
            request,
            "followups.html",
            {"run_id": selected_run, "timeline": timeline, "outreach": timeline},
        )
    ...
    async def register_response(request: Request) -> RedirectResponse:
        ...
        try:
            ...
            Workflow(repository).register_response(
                run_id=run_id,
                outreach_action_id=_required(form, "outreach_action_id"),
                responder_subject_key=_required(form, "responder_subject_key"),
    ...
    async def register_interview(request: Request) -> RedirectResponse:
        ...
        try:
            ...
            Workflow(repository).register_interview(
                ...
                completed_at=_required(form, "completed_at"),
                summary={"summary": _required(form, "summary")},
    ...
    def metrics(request: Request, run_id: str | None = None) -> HTMLResponse:
        ...
        try:
            selected_run = run_id or _latest_run_id(repository)
            snapshot = (
                MetricsEngine(repository.connection).calculate(selected_run)
                if selected_run
                else None
            )
        ...
        return templates.TemplateResponse(
            request,
            "metrics.html",
            {"metrics": snapshot, "run_id": selected_run},
        )
    ...
    *,
    ...
    statement = """
        WITH ranked_scores AS (
          SELECT sr.*,
                 ROW_NUMBER() OVER (
                   PARTITION BY sr.mvp_run_id, sr.signal_id
                   ORDER BY sr.created_at DESC, sr.score_run_id DESC
                 ) AS rank
          FROM score_runs sr WHERE sr.status = 'SUCCEEDED'
        ), leaf_reviews AS (
