...
def capability() -> dict[str, Any]:
    ...
    if not endpoint or not token:
        ...
        status = "REQUIRES_PROOF"
        note = "已配置搜索 API，但尚未证明结果 URL 可重开、发布时间可核验和保存边界。"
    else:
        status = "READY"
...
def _normalize_item(item: Any, provider: str) -> dict[str, Any]:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise SearchConnectorError("provider_item_invalid", "provider source_url must be an HTTP URL")
    return {
        "title": title[:240],
...
class AuthorizedSearchConnector:
    ...
    def search(self, plan: dict[str, Any], mode: str = "quick") -> list[dict[str, Any]]:
