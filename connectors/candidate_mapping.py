...
def _xhs_post_record(content: object, collector_version: str, query: str | None) -> dict:
    if not isinstance(content, Mapping):
        raise CandidateMappingError()
    converter = lambda value: value if isinstance(value, str) and _XHS_ID.fullmatch(value) else _numeric_id(value)
    source_id = _aliases(content, ('note_id', 'source_id', 'id'), converter)
    if source_id is None:
        raise CandidateMappingError()
    ...
    return dict(kind='POST', external_source_id=source_id, external_comment_id=None,
        public_url=url, title=title, body=body,
        author_public_id=_author(content, ('user_id', 'sec_uid', 'uid')),
        published_at=_aliases(content, ('time', 'create_time', 'published_at'), _time),
        observed_at=_time(content.get('collected_at')), parent=None,
        collector_version=collector_version, normalizer_version='raw-xhs-post-candidate-v1', query=query)
...
def _record(platform: str, raw: object, collector_version: str, query: str | None) -> dict:
    ...
    if platform == 'XIAOHONGSHU' and set(raw) == {'content'}:
        return _xhs_post_record(raw['content'], collector_version, query)
