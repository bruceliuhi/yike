...
### 已批准的临时访问码（migration 138）

用户已批准受邀临时访问码作为短信投递阻塞期间的独立登录方式。先正常登记客户，在客户列表选择“签发/改发临时登录码”并单独确认，再人工私下发送当次唯一展示的 `YA-` 访问码。客户仅在软件临时登录表单填写码；不要放入 URL、日志或普通客户端存储。原 `YK-` 短信试用码永远不能独立登录。未激活短信 trial 可显式转换；已激活短信 trial 不转换。临时码可显式改发，旧码立即失效，但既有会话须由退出/停用/到期门禁控制。

首次成功访问码登录固定开启 72 小时，再登或改发不延长。停用或到期后，旧会话也被拒绝；cookie 和签名有效期不超过剩余试用时间。预登记手机号不等于验证，后台只按真实 OTP 成功后写入的 `phone_verified_at` 显示已验证，不根据 activated_at 推断历史记录。首次临时激活后，同一手机号后续真实短信验证可登录原用户，保留其到期时间与数据。

升级顺序：先认证加密备份，再以受信管理员执行全部迁移（含 138），对既有受限客户角色执行 `grant_runtime.sql`（包含 `grant_temporary_access.sql`），对独立运营角色重新执行 `grant_ops.sql`；必要的数据库 CONNECT 仍只向各自角色授予。先升级并验收客户服务，再升级运营服务，最后才允许运营显式签发/转换；不要将新运营发码与旧客户服务混用。本实现不自动部署或修改生产权限。
