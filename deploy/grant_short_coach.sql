DO $$ DECLARE target TEXT:=NULLIF(current_setting('yike.app_role',true),''); role_oid OID; BEGIN
 SELECT r.oid INTO role_oid FROM pg_roles r WHERE rolname=target AND NOT rolsuper AND NOT rolbypassrls AND NOT rolcreaterole AND NOT rolcreatedb;
 IF role_oid IS NULL THEN RAISE EXCEPTION 'restricted application role required'; END IF;
 ...
 IF EXISTS (SELECT 1 FROM pg_class WHERE oid='public.pilot_material_revisions'::regclass AND pg_has_role(role_oid,relowner,'MEMBER')) THEN RAISE EXCEPTION 'application role must not own material revisions'; END IF;
 EXECUTE format('GRANT SELECT ON public.pilot_material_revisions TO %I',target);
