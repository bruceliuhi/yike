...
try {
  ...
  if (-not $nodeCommand -or -not $nodeVersion -or -not $nodeVersion.StartsWith('24.')) {
    ...
    $report = [ordered]@{
      ...
      outcome = 'BUILD_FAILED'; failureCode = $(if ($nodeCommand) { 'NODE_24_REQUIRED' } else { 'NODE_NOT_FOUND' }); manualAcceptance = 'UNTESTED'
    ...
    Write-Output ('Node.js 24 x64 is required. Failure evidence: ' + $evidenceDirectory)
    exit 1
  }
