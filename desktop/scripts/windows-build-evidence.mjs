...
export function runWindowsBuild(root) {
  const evidence=createEvidence(root);let failureCode=null;let runtime;
  const run=(id,operation)=>{beginStage(evidence,id);let code=1;try{code=operation();}catch(error){if(id==='preflight'&&error instanceof BuildRuntimeError)failureCode=error.code;}endStage(evidence,id,code);if(code!==0){failureCode||=id==='preflight'?'WINDOWS_X64_NODE24_15_REQUIRED':'STAGE_FAILED';throw new Error('STAGE_FAILED');}};
  try{
    run('preflight',()=>{
      ...
      runtime=createWindowsBuildRuntime({cwd:evidence.root});recordRuntime(evidence,runtime);return 0;
    });
