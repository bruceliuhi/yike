...
const privateExecutionSchema = z.discriminatedUnion('operation', [
  ...
  z.object({operation:z.literal('execution.support'),samplingVersion:z.literal(1).optional(),progressVersion:z.literal(1).optional()}).strict()
...
export function validatedExecutionOperation(input: unknown): ServiceOperation | null {
  ...
  switch (request.operation) {
    ...
    case 'execution.support': return {path:'/api/ui/execution-support'+(request.samplingVersion===1?'?sampling_version=1':request.progressVersion===1?'?native_progress_version=1':''),method:'GET',logout:false};
