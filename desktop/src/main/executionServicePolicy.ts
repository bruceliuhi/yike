...
const privateExecutionSchema = z.discriminatedUnion('operation', [
  ...
  z.object({operation:z.literal('execution.support'),samplingVersion:z.union([z.literal(1),z.literal(2)]).optional(),progressVersion:z.literal(1).optional()}).strict()
    .refine(value=>value.samplingVersion===undefined||value.progressVersion===undefined),
...
export function validatedExecutionOperation(input: unknown): ServiceOperation | null {
  ...
  switch (request.operation) {
    ...
    case 'execution.support': return {path:'/api/ui/execution-support'+(request.samplingVersion!==undefined?`?sampling_version=${request.samplingVersion}`:request.progressVersion===1?'?native_progress_version=1':''),method:'GET',logout:false};
