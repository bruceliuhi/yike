...
function recordsFrom(value:unknown,keywords:string[],exclusions:string[],maximum:number,observed:number):CandidateSubmission['records'] {
  ...
  for(const topic of value.slice(0,maximum)) {
    ...
    records.push({kind:'PAGE',external_source_id:String(topic.id),external_comment_id:null,
      ...
      collector_version:'v2ex-latest-v1',normalizer_version:'v2ex-page-v1',query});
  }
    ...
    const completed=Promise.resolve().then(async()=>{
      try {
        ...
        try {
          ...
          return recordsFrom(JSON.parse(body),configuration.keywords,configuration.exclusions,input.maxRecords,now());
        }catch {
