...
export interface SearchSuggestionsService {
  preview(profileVersionId:string):Promise<SuggestionPreview>;
  submit(request:SuggestionRequest):Promise<SuggestionReceipt>;
  getReceipt(request:SuggestionRequest):Promise<SuggestionReceipt>;
}
...
export function createSearchSuggestionsService(transport:Transport):SearchSuggestionsService {
  return {
    async preview(profileVersionId){
      ...
      const value=checked(suggestionPreviewSchema,await transport('suggestions.preview',
        `/search-suggestions/preview?profileVersionId=${input.profile_version_id}`,'GET',input));
    ...
    async submit(input){
      ...
      return receipt(await transport('suggestions.submit','/search-suggestions','POST',request),request);
    },
