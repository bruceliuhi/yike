...
function fixture(){
  ...
  const identity={getStatus:()=>({state:'READY' as const,deviceId,credentialVersion:1}),openWorkerScope:async()=>{const abort=new AbortController();return {ok:true as const,scope:{session:{userId,sessionId,isCurrent:()=>current},device:{deviceId,credentialVersion:1},signal:abort.signal,transport:{requestOutreach:service.requestOutreach,requestConnection:connection,requestCandidate:service.requestCandidate,requestExecution:service.requestExecution},close:()=>abort.abort()}};}};
