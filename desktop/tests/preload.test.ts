...
describe('preload export bridge', () => {
  it('exposes a frozen fixed API and forwards only the export payload to its fixed channel', async () => {
    ...
    expect(Object.keys(api).sort()).toEqual(['copyText','executionCommand','foregroundCollectionCommand','getClientInfo','getDeviceIdentityStatus','getPortableRuntimeStatus','getRuntimeStatus','monitorCollectionCommand','nativeOutreachCommand','nativeReplyCommand','openExternal','openSourceView','platformConnectionCommand','prepareDeviceIdentity','requestApi','saveExport']);
    ...
    await api.monitorCollectionCommand!({action:'LIST'});
    expect(mocks.invoke).toHaveBeenCalledWith('desktop:monitor-collection',{action:'LIST'});
