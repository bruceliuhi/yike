...
it('offers an empty-evidence ledger request and sends only the exact sync payload',async()=>{
  ...
  fireEvent.click(screen.getByRole('button',{name:`同步原请求 ${requestId}`}));
...
it('deduplicates ledger and evidence requests and refreshes after partial success or failure',async()=>{
  ...
  expect(screen.getAllByRole('button',{name:`同步原请求 ${requestId}`})).toHaveLength(1);
  fireEvent.click(screen.getByRole('button',{name:`同步原请求 ${requestId}`}));await screen.findByText(/部分范围读取/);expect(reload).toHaveBeenCalledOnce();
...
it('does not expose an id field and explains missing originals and desktop bridge',()=>{
  ...
  expect(screen.queryByRole('textbox')).toBeNull();expect(screen.getByText(/需先完成并核实原生联系/)).toBeTruthy();view.unmount();
...
it('never calls native sync for sample, logged-out, or non-XHS opportunities',()=>{
  ...
  expect(screen.queryByRole('button',{name:/同步原请求/})).toBeNull();expect(command).not.toHaveBeenCalled();
});
...
it('treats a mismatched or malformed result as unconfirmed and does not reload',async()=>{
  ...
  fireEvent.click(screen.getByRole('button',{name:`同步原请求 ${requestId}`}));await screen.findByText('本次同步结果未确认，请稍后核对保存证据。');expect(reload).not.toHaveBeenCalled();
});
