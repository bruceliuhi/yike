...
export function configureCandidateReviewVisual(
  ...
  if (!mode || !["live", "recovery", "raw-error"].includes(mode)
    ...
    throw new Error("TEST 候选场景仅接受 scenario=P07、candidateReview=live/recovery/raw-error 与可选 state=populated。");
  ...
  const api = createCandidateReviewService(async (operation, _path, _method, payload) => {
    ...
    if (!("action" in request)) {
      result = parseCandidateReviewResult({ kind: "sourceVerification", requestId: request.requestId,
        ...
        locator: request.locator, excerpt: request.excerpt, contactMethod: request.contactMethod }, { requestId: request.requestId, request });
      ...
      result = parseCandidateReviewResult({ kind: "assessment", requestId: request.requestId, candidateId: request.candidateId,
        assessment: { ...assessmentFixture(), assessedAt: createdAt } }, { requestId: request.requestId, request });
