...
export function createVisualService(
  state: VisualState = "populated",
  guest = false,
) {
  ...
  const sessionWrite = (name: string) => {
    ...
    if (!session.authenticated)
      throw new ServiceError("UNAUTHORIZED", "TEST 请先登录", 401);
    if (state === "error")
      throw new ServiceError(
        "VISUAL_TEST_ERROR",
        "TEST 保存失败；输入保留",
  ...
  const service: YikeService = {
    ...
    outreach: {
      queue: (queue) =>
        read(
          "outreach.queue." + queue,
          {
            queue,
            items: [
              {
                id: "TEST-outreach-" + queue,
                ...
                recipientLabel: "TEST 内存对象",
                content: fixtures.opportunity.dm,
                updatedAt: fixtures.TEST_TIME,
                sample: false,
      ...
      send: async () => {
        record("outreach.send", "TEST 拦截发送");
        throw new ServiceError(
      ...
      reconcile: async (request) => {
        record("outreach.reconcile", "TEST 只读核对；无真实请求");
    ...
    saveProfile: async (fields) => {
      ...
      const saved: Profile = {
        id: `TEST-profile-v${profiles.length + 1}`,
        version: profiles.length + 1,
        status: "DRAFT",
        fields: structuredClone(fields),
        description: "TEST 内存画像",
      };
      profiles = [saved, ...profiles];
      return structuredClone(saved);
    },
    confirmProfile: async (id) => {
      sessionWrite("confirmProfile");
      const p = profiles.find((item) => item.id === id);
      if (!p) throw new ServiceError("NOT_FOUND", "TEST 画像不存在", 404);
      p.status = "CONFIRMED";
    ...
    addFollowup: async (id, status, note) => {
      sessionWrite("addFollowup");
      const row = findOpportunity(id);
      followups.push({
        id: `TEST-followup-${followups.length + 1}`,
        opportunityId: id,
        title: row.title,
        status,
        note,
        kind: "manual",
        createdAt: fixtures.TEST_TIME,
      });
    },
    connections: () => read("connections", fixtures.connections, []),
    connect: async (platform) => {
      record("connect", `TEST ${platform}；未打开登录窗口`);
    },
    checkConnection: async (platform) => {
      record("checkConnection");
      const c = fixtures.connections.find((item) => item.platform === platform);
      if (!c) throw new ServiceError("NOT_FOUND", "TEST 平台不存在");
      return structuredClone(c);
    },
    disconnect: async (platform) => {
      record("disconnect", `TEST ${platform}；未改变任何真实账号`);
    },
    suggest: async (profileId, requestId, signal) => {
      record("suggest", "TEST 固定建议；未调用模型");
      ...
      return read(
        "suggest-result",
        {
          ...
          exclusions: fixtures.EXCLUSIONS,
        },
        { profileId, requestId, keywords: [], exclusions: [] },
      );
    },
    tasks: () => read("tasks", tasks, []),
    startTask: async () => {
      record("startTask", "TEST 拦截启动，无执行器");
      throw new ServiceError(
        "CAPABILITY_UNAVAILABLE",
        "TEST 不启动真实或模拟执行器",
        501,
    ...
    taskAction: async (id, action) => {
      sessionWrite("taskAction");
      const task = tasks.find((item) => item.id === id);
      if (!task) throw new ServiceError("NOT_FOUND", "TEST 任务不存在", 404);
      task.status = {
        pause: "PAUSED",
        resume: "RUNNING",
        cancel: "CANCELED",
    ...
    generateContact: async () => {
      record("generateContact", "TEST 固定草稿；未调用模型");
      return fixtures.opportunity.comment;
    },
    saveContact: async () => {
      sessionWrite("saveContact");
    },
    verifyContact: async (draft, fingerprint): Promise<ContactVerification> => {
      ...
      return {
        ...
        recipientId: "",
        recipientLabel: "TEST 无真实收件人",
        reason: "TEST 视觉夹具禁止消息发送",
      };
    },
    send: async () => {
      record("send", "TEST 拦截发送");
      throw new ServiceError(
        "CAPABILITY_UNAVAILABLE",
        "TEST 消息没有发送",
        501,
    ...
    openExternal: async () => {
      record("openExternal", "TEST 已拦截外链；未打开浏览器");
    },
    copy: async (text) => {
      record(
        "copy",
        `TEST 仅记录 ${text.length} 个字符的复制请求；未修改剪贴板`,
      );
    },
    candidates: (query) => {
      ...
      const matches =
        ...
        (!query?.query || item.title.includes(query.query)) &&
    ...
    reviewCandidate: async (review): Promise<CandidateReviewResult> => {
      ...
      return {
        ...
        assessment: {
          ...structuredClone(fixtures.candidate.assessment!),
