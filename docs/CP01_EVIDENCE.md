...
代码备份：Gitee `codex/customer-pilot` 分支；当前 HEAD 为 `973d35513a71409a2e52b85b5cec2d96348174de`，已与远端 SHA 对齐；相关部署提交包括 `eff28d7`（固定 Python 基础层 digest）、`bd27338f108652e9ec43bdb4ea24fdfe4935cc77`（非 root 运行）、`82ce5c0b284dfab5eeca5ef0999618427855a60f`（备份 HMAC 完整性）、`671964f`（CP-06 生产配置 preflight）、`6f61c98`（受限 Compose 运行文件）与 `973d355`（根构建上下文及只读运行修复）。产品门禁现包含备份篡改拒绝测试和 preflight 单元测试。更早的部署验收模板及 HTTPS-only `/healthz`、`/readyz` 探针提交为 `006ac86188e366645b002d84ca274ddde33ed447`；探针对 HTTP、userinfo、query、fragment 输入均 fail-closed。浏览器主流程记录见 `docs/BROWSER_ACCEPTANCE_CP04.md`；该分支尚未部署。
