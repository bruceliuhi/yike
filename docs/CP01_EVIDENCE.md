...
结果：CP-01/02/03 数据层与研究导入测试通过；最新完整命令（含四页 Web 与导入门禁）为 `10 passed in 1.96s`（含真实 PostgreSQL 集成测试；集成测试创建一次性非超级用户 `pilot_app`，验证 RLS 过滤与旧约束升级）。
...
代码备份：Gitee `codex/customer-pilot` 分支，当前最新提交 `9bfd2ba81e117d87f0e2865bf70e753b7f21ebde`。浏览器主流程记录见 `docs/BROWSER_ACCEPTANCE_CP04.md`；该分支尚未部署。
