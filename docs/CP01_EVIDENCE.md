...
结果：CP-01/02/03 数据层、任务租约、四页 Web、来源状态（含未核验提示）、失败任务提示、研究导入、受信 provisioning 和日志脱敏测试通过；最新完整命令为 `20 passed in 2.58s`（含真实 PostgreSQL 集成测试；集成测试创建一次性非超级用户 `pilot_app`，验证 RLS 过滤与旧约束升级）。
...
代码备份：Gitee `codex/customer-pilot` 分支；最近一次完整回归绑定提交 `057532c`，部署骨架已推送至 `2f8c439e5878cbada47b6354db52d0385279b3d6`。浏览器主流程记录见 `docs/BROWSER_ACCEPTANCE_CP04.md`；该分支尚未部署。
