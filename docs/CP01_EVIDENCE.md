...
结果：CP-01/02/03 数据层、任务租约、四页 Web（含来源状态、更新时间和匹配摘要）、失败任务提示、研究导入、受信 provisioning、日志脱敏、迁移职责、HTTPS 会话入口和代理头信任边界测试通过；最新完整命令为 `25 passed in 3.67s`（含真实 PostgreSQL 集成测试；集成测试创建一次性非超级用户 `pilot_app`，验证 RLS 过滤与旧约束升级）。
...
代码备份：Gitee `codex/customer-pilot` 分支；最近一次完整回归绑定提交 `ad7cc47`，部署骨架已推送至 `2f8c439e5878cbada47b6354db52d0385279b3d6`。浏览器主流程记录见 `docs/BROWSER_ACCEPTANCE_CP04.md`；该分支尚未部署。
