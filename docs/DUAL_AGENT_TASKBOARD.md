...
保留 V02-01～10 主任务编号，下列后缀只拆分交付，不增加产品范围。`V02-01A/B` 沿用 Mac 已使用的真实子项编号，其余是待认领的切片；创建卡片不等于已分派或已开始。旧 `V02-02-WIN` 拆为 `V02-02C` 与 `V02-02D-*`，旧整项 V02-05/V02-08 不再整体分配给 Win。
