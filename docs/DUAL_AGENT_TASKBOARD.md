...
**当前执行目标（2026-09-10 最新决定）：** 用户要求按原规划完整 V0.2 全量推进，下文全部首发任务恢复为当前交付目标，依据见[产品计划第1.0节](V02_COMMERCIAL_RELEASE_PLAN.md#10-当前执行完整-v02-全量推进2026-09-10)。分阶段可验收版本与100候选/30联系只作中间验证。CodexiMac已恢复本轮接续；CodexWin的运行状态和待审核来件按[实施任务书顶部](V02_IMPLEMENTATION_TASKBOOK.md)记录，不代称另一任务已启动或已ACK。已有实现复用，未合并工作逐片审查后接收。
...
- 默认负责 `main` 合并、迁移、服务端集成和发布判定；用户明确指定的提交由指定执行者完成。2026-09-09 文档/目标调整曾由 CodexWin 提交 main，本次2026-09-10完整目标恢复由 CodexiMac 接续；按仓库工作流执行小改直接 main、较大或并行工作使用短期分支，脏工作树隔离保留。
...
建议主责一列只有一个交付责任人；Mac 指 CodexMac/CodexiMac，Win 指 CodexWin。用户追加分担后04A/B实际主责改为Win，与05B/C组成纵向功能交付，Mac已接收分工，精确边界/108预留见唯一任务书第7节。另一端或未参与该实现的独立 Agent 审核，不能自审；实际接收人与 reviewer 在认领时登记。涉及同一契约/页面的卡片先明确文件边界再并行。小范围串行改动直接使用最新 main；确需短期分支时再从最新 main 建立并记录，不预先把建议名称当作存在的分支。
...
| V02-04A / Win，Mac接收 | 多业务画像与资料提取契约/服务；独立模块复用 `pilot/store.py`、`pilot/ui_api.py`，共享入口按任务书串行集成；定义资料格式、失败补录与版本确认 | 可先设计/测试；保存与鉴权使用现有 pilot 基础及已接收的身份契约 | 多产品/服务、客群、地域、供应能力、销售方式、规模/周期及排除项可保存确认；输入权限/大小/类型、来源引用及版本反例 |
| V02-04B / Win，Mac接收 | 行业/销售方式策略服务与评测集；`skills/ai-project-lead-research-v1/`、新增 `pilot/` 研究模块 | V02-04A 的画像契约；规则与五类业务反例可先做 | 来源/需求表达/搜索排除/判断/预算输出绑定版本；人工修改受保护；有真实模型运行及失败证据，模板不是行业允许名单 |
...
| ID / 建议主责 | 小卡交付及代码入口 | 实现/联调依赖 | 本卡完成证据 |
|---|---|---|---|
| V02-02B / Mac | 受认证原始候选上传 API 与短事务存储；`pilot/ui_api.py`、独立候选模块、`migrations/`；复用已有原子导入测试模式 | V02-01C、V02-02A；Win按[五字段准备/原键恢复合同](contracts/V02_RAW_CANDIDATE_INBOX.md#11-普通客户端候选签名准备2026-09-10)接候选上传 | 同键重放/异内容冲突/整包回滚/租户解析/旧代次拒绝；准备不产生事实、不作为历史恢复的强制前置；桌面不能上传 APPROVED 或运行管理员导入 |
...
2. **可独立认领：** Win 的 V02-04A/B及05B/C、V02-02C、V02-09A（随后 V02-09B 的本地进程部分）；Mac 的 V02-01C、V02-02A、V02-06A、V02-07A、V02-10A。先检查已有认领和完成记录；列表不是同时启动所有卡的指令。
