...
2026-09-09 身份/会话与测试基线集成：远端 main 已包含 `5d3373d9b7fa6cb74f0a9f6241bbb282e4db2514`，即独立审核通过的01A登记/遥测、01B会话撤销、104/105最小权限升级和V02-10E测试时钟修复。保留最新跨行业目标、细化任务卡与R3前端；该版本本轮后端705项（含专用PG、无跳过）、桌面362项及typecheck/build通过，独立合入复审PASS。实际Win接口接收、设备认证、正常登录、真实平台、Windows发行和生产/UAT未完成；[组合验收](qa/IDENTITY_INTEGRATION_20260909.md)区分代码集成与接收ACK。以下为历史整合快照，不覆盖此项更新。
...
上述历史版本的完整 Python 回归有日期窗口相关失败；V02-10E 后续修复已随 `5d3373d` 合入，最新完整705项通过见开头记录。不能将历史失败解释为新版本仍失败，也不能把新版通过回填成旧版当时已经通过。
