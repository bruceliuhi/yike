...
2026-09-10 Mac原确认回执补齐候选`9d9e965`（base `7b640c9`）：仅新决策返回原来源核验ID，旧持久回执不回填；不新增SQL/授权，不改Win客户端。7项实际HTTP/受限PG与46项纯边界分别通过，[本次QA](qa/V02_CANDIDATE_ASSESSMENT_REVIEW.md#05g原确认回执补齐2026-09-10)保留RED和测试修正；整批/独立审核待记录，实际Win消费、来源/发送/上线未验。Mac下一处接设备登记未知恢复最小合同；首发仍按已批准端到端及三个亮点推进，高级管理/复杂统计/重复全量测试后排，基础隔离/确认防重/恢复不后排。
