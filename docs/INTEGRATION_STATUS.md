...
## 2026-09-26 Lead Radar 收敛到现有意客AI

用户决定：整合 Lead Radar，前端统一且简洁。客户入口继续使用现有桌面客户端、账号、画像、任务、线索、商机和监控页面，不新增第二个客户工作台。

| 能力 | 本轮归属与实现 |
| --- | --- |
| 搜索规划 | `pilot/radar_plan.py` 是共享纯内核；原 `apps/lead_radar/planner.py` 兼容调用它，实际 Codex 研究上下文也使用它。只从当前条件展开，不补入 AI 行业默认词。 |
| 计划预览 | 原任务向导的公开网页自主研究中，按需展开搜索计划；固定 `/api/ui/research-plan/preview` 使用原客户会话，不调用外部模型/来源，不另建任务、数据库或用量账。 |
| 执行与计量 | 继续使用现有签名策略、`pilot_collection_tasks`、云端研究 broker、资源账本和搜贝规则；不把独立 worker 的 SQLite 状态或另一套定价混入客户执行。 |
| 结果与复核 | 使用现有 PG 候选、原文版本、复核、商机、草稿与跟进；采集和监控详情直接进入当前任务线索。原文元信息折叠，失败及未知保留。 |
| 首页简报 | 在当前本人/画像范围解释待补证原因与下一步；已排除、已关闭、已联系的记录不因缺证再次推荐。 |
| 运行界面 | 沿用 R3/R4 导航；首页只保留一个样例入口，完整样例仍在商机库，运行日志和详细计数按需展开。 |

独立目录中的实体手工合并/拆分、校准评测导入、独立业务 API/MCP 和来源供应商试验工具仍属内部工具，未接入客户界面的项目不计为客户已交付功能。现有产品的数据和来源核验门槛是权威；不直接搬入独立模块更宽松的评分或权限规则。本轮检查 `apps/lead_radar` 下没有遗留 SQLite 数据库文件，不需要迁移该目录的客户数据；未扫描或改写其它路径数据。

本轮无数据库迁移、无新收费参数；研究规则指纹加入 `radar-search-directions-v1`。生产更新前应停止新研究准入、等待既有运行结束，随后更新服务与客户端，不强行续用旧绑定任务。验证与发布状态见唯一[实施任务书](V02_IMPLEMENTATION_TASKBOOK.md)，提交不等于已部署。

## 2026-09-13 首次试用收尾：原文可见与引用工具通道

`2b118fa/00f35ba`已接成功READ只读API、现有进度页原文展开及来源URL；`f343a64`修复真实Codex结构化工具通道，独立差量审核PASS。正常合入远端`a0142c7`为`5426a27`并保留设备自动准备/可选MCP隔离。绑定该合并版的真实模型同快照检查通过：3MODEL/2READ、COMPLETED、两页逐字引用与绑定通过，目录/供方保留为背景，候选0不虚报商机。[本批唯一代码、两次失败、实际成功与限定界面证据](superpowers/plans/2026-09-13-research-efficient-handoff.md#evidence)。下一步统一同版本客户端/部署和新买方试用验收，不再重修已通过的引用通道；未部署/构包/新商机/外联，完整Goal ACTIVE。下方为历史接续记录。
