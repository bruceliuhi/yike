...
运营后台与三天试用：新增 `/ops` 独立服务，管理员查看完整手机号、签发/重发/停用专属试用码；客户真实短信验证后激活，默认72小时。[运行与部署说明](../deploy/OPS_ADMIN.md)，[实施与限定证据](plans/2026-09-11-ops-trial-admin.md)。代码交付不等于已部署或短信收码完成。正式阿里云验证码接线与公开模板配置见 [短信部署说明](../deploy/ALIYUN_SMS.md)；临时后四位方案已暂停，不作为当前登录路径。
