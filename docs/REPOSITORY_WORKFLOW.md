...
## 快速可验收候选（2026-09-10 用户要求）

- 以可操作的端到端增量组织交付；小修改只跑受影响定向测试，整批做一次独立审核，不逐文件重复全量审核。
- 审核修复只复核差量及受影响边界；同一源码字节复用已验证证据。纯文档或不影响产品的合并不触发构包、全量测试。
- 可见入口完成并冻结后构建一份候选，复用同包验收；仅产品/构建输入变化才重新构包。整链验收留到发布阶段，不逐小卡重复执行。
- 每批只保留简短的版本、改动、验证结果、未验范围和下一步；任务书引用证据，其他交接文档不复制长报告。
- 原文证据、账号/数据隔离、人工确认、防重复发送和未知结果恢复仍是底线；未通过或跳过的验证不能写成通过，候选不等于上线。
