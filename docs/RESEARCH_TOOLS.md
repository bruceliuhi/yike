...
受控模式的 MCP 原文工具只拿临时 loopback 地址与令牌，经宿主验证“该 URL 确由本任务搜索发现”，再由本任务专属读取器读取。不同任务关闭互不取消。已处理的查询、原文及失败尝试不重复外部调用；宿主可拒绝、缩短期限或返回已存结果，不能重复启动同一动作、延长期限或返回后再启动动作。配置缺失、非法、拒绝不回退到直连。

已用本地真实 HTTP/stdio 协议和 fixture Codex 进程验证控制链，外部模型/搜索/网页使用测试替身；**不是公网研究、客户 UAT、持久计费或上线证明**。[本批唯一证据](superpowers/plans/2026-09-13-research-effect-gateway.md#evidence)。下一步仍需同一 PostgreSQL 任务许可与动作账本、客户上下文 v2、主管执行链和证据入库；完成前不开放新的客户动态研究能力。
