...
适用范围：旧 `app/` 双平台 Discovery / SQLite 参考实现，供复用与排查使用；不是当前客户产品的范围权威或 V0.2 运行手册。当前目标与任务以 [AUTHORITY](../AUTHORITY.md) 和 [V0.2 实施任务书](V02_IMPLEMENTATION_TASKBOOK.md) 为准；现有客户 `pilot/` 的操作见 [客户试用运行手册](CUSTOMER_PILOT_RUNBOOK.md)。下文双平台限制、人工联系和手动重试仅描述旧实现，不取消 V0.2 多平台持续监控与确认后真实触达任务。
