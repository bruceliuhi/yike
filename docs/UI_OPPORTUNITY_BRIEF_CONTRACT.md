...
[隔离视觉适配](../desktop/tests/visual/r4-opportunity-brief.ts) 只接受 `TEST` 会话范围，返回既有隔离样例对象的内存快照；动态有效期用于真实页面交互验收。它不访问外部平台、不写客户库、不生成发送或启动回执。生产简报服务、稳定业务日策略与真实数据聚合仍由正式服务适配提供；本合同不等于这些能力已经上线。
