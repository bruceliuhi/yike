...
普通服务先 `preview(input)` 返回原输入摘要及实际模型标识，界面展示完整公开原文、当前人工草稿、选定业务资料片段与模型名称；明确确认后以同一请求加 `disclosure` 调用生成。服务端核对已保存原文、画像、来源及资料当前资格；模型仅获得原文/草稿/channel/purpose及选定编号片段，不带账号、收件对象或其他资料库内容。取消停止客户端等待，不自动重试；同原请求最多调用一次，重放只返回旧结果或明确状态。安全上限为每租户UTC自然日50次新调用，失败计入，不是价格或搜贝。建议有效期5分钟，承诺仍需人工复核。
...
目的为 `requirement`（具体需求）、`materials`（资料获取）、`scope`（服务范围）。可选 `materialReferences` 复用联系草稿出处结构，最多3条；每条必须在人工草稿中，且为本人最新 READY/external 资料原文子串。内部资料及未选定正文不发送。非空引用采用 `short-coach-material-draft-v1`，无引用保留旧政策；缺字段的旧14项哈希不变，显式空数组和非空数组分别追加到摘要。

模型仅见 `materialQuotes: [{referenceIndex, quote}]`；生成内容中的实际引用可缩短、可选子集，出处由服务端按输入重建，模型不能编造资料身份。返回 `materialReferences` 与正文同步采用和保存；空/缺字段分别兼容原输入。采用前再次只读 preview 核验相同授权与当前资料，任何失配保留人工稿；之后保存及发送仍独立核验，不能将一次核验承诺为永久授权。部署需应用更新后的 `deploy/grant_short_coach.sql`（资料最小 SELECT），模型适配器须声明资料政策能力，否则调用/配额预留前拒绝。[本批证据与边界](superpowers/plans/2026-09-11-material-short-coach.md#实施与验证)集中记录，不等同真实模型质量或上线。
