...
| V02-05A / CodexMac，保留原前端工作及主线交接记录 | IN_PROGRESS | 最新 16a9a3f 正常保留 Win 持钥模块，已验前端运行路径仍与 f18 相同；完成 P04 资料隔离、P12 双渠道草稿保护、P14/P15 原操作及纠正历史恢复；16a 合并全量 1206 passed / 22 skipped；f18 产品候选类型/构包/严格 smoke 通过。新 Mac 包冷启动及平台状态已验，详情见[状态恢复验收](qa/ui-state-recovery/README.md)。旧 [R4](qa/ui-r4/README.md)、[20 页基线](qa/ui-final-acceptance/REVIEW.md)及[整合验收](qa/ui-reviewed-integration/README.md)保留原证据范围，不累加测试 | 继续剩余同状态视觉、原生退出/重启/选择器及 Windows 验收；补无人工记录的匹配回复入口 P2。真实后台、平台与计量收费按原 05B～G/07C/08B/09C/E 接入，不把 TEST 或未提交 raw 草稿计作真实能力，不将 05A 或 Goal 标 DONE |
