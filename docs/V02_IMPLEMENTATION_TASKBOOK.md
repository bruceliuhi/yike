...
02/MP03知乎接续：原POST/COMMENT数据契约之上已复用固定MediaCrawler接登录、本人账号护栏、受控请求和总预算；`0cbe2a9`非作者整批GO。[唯一实施与限定证据](superpowers/plans/2026-09-11-zhihu-controlled-runtime.md#实施与验证)。下一里程碑集中做四平台显式配置与普通任务/监控接线、可用服务配置、同版本Windows包和真实搜索→入库→客户端详情；不继续孤立适配或重复整套测试/构包。当前知乎尚未在客户端启用，不以合成检查证明真实采集；完整Goal其它范围保持。
