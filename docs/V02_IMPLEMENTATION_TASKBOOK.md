...
2026-09-13 Mac交付检查：首个make未显式绑定服务地址，不作为可用候选；已用YIKE_RELEASE_SERVICE_URL=https://yike.tuokexing.net重构一次（70765 exit0），ASAR主入口确认正式地址。当前ZIP SHA256 `24f80c5977e014e6c9b9610791703e2ad96c6998fe46c32493e277c5426c8285`，产品源码d19dade，合并64770a1仅追加双方文档。旧客户端正常退出请求后窗口消失，但主进程57815仍存活（mac-20260911旧包），新候选两次窗口连接超时；没有强制终止、删除登录或草稿。故新包简报修复未取得实机通过。需用户同意结束旧进程或自行完全退出后再启动候选；不把无窗口当已退出，保留真实短信登录成功与新包未验的区别。
