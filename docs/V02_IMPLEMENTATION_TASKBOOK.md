...
|---|---|---|
| P0 / 02、03、05F/G、09B/C | Win：Windows运行时路径/私有目录/进程清理、真实worker、上传批次恢复和既有任务入口；Mac：小红书映射及候选/执行后端 | IN_PROGRESS；上传674eafc、进程清理e520850、原批次恢复0263d49已交付；Windows开发用自动安装已实际走通新目录/固定依赖/CLI/本地Chromium/整树权限校验，见[交接](handoffs/V1_WIN_FUNCTION_OWNERSHIP_20260909.md#分工调整)。客户bootstrap、真实worker/身份epoch装配与三渠道采集未验，Mac在途映射不并改 |
