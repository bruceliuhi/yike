...
运营试用后台：本批新增136、独立受限`/ops`服务、管理员完整手机号/签发/重发/停用、真实SMS后原子激活默认72小时、旧会话权益gate及客户端解释。[实现与限定证据](plans/2026-09-11-ops-trial-admin.md)、[部署配置](../deploy/OPS_ADMIN.md)。99项后端＋27项独立RLS会话＋32项前端及tsc通过，尚待非作者审核/集成；无真实短信供应商或生产部署证据。主线程的临时受邀登录设计是下一批，不以本批SMS激活字段冒充未验证手机身份。
