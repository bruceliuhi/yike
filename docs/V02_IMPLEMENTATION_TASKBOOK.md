...
本次同步基线为 `origin/main@03f2705`。Win 暂停分支 `codex/win-collection-flow@96bb371` 的离线运行包及回复修复尚未合入 main；Mac 原 `codex/launch-resume` 工作树的运行包在途改动原样保留，不混入本次文档提交。Mac 先负责范围同步、接收审核及后端闭环；Windows运行时、客户端/平台入口和业务画像/策略的既有Win文件边界保留，接续前核对来件，不双写。独立审核由非作者执行；真实账号挑战及发送的对象/内容/渠道确认仍保留，不因恢复开发而扩大外部操作授权。
...
| P0 / 06、07、08 | Mac：接收审核回复WIP、发送/回执/回复后端；Win：渠道与现确认/跟进页面 | IN_PROGRESS；接真实判断、独立短草稿、确认发送与原回执核对，再接真实回复采集关联。人工登记与平台回复分别记录；自动回流不是未经确认自动回复 |
