...
2026-09-13 22:30：70b1bd6 已推 main、构包并实装，修复小红书默认热门排序及遗漏原帖候选的问题，帖子与评论共同遵守条数上限。独立差量 GO，定向与运行时证据见[本批唯一记录](superpowers/plans/2026-09-13-xhs-fresh-demand.md)。新版保留登录，普通任务已填写但尚未启动；窗口前台激活连续两次失败（客户端仍 Responding），已停止输入等待恢复后继续同一草稿。新鲜有效需求及草稿跟进尚未验收；不重复构包、不重启旧付费研究，完整 Goal ACTIVE。
