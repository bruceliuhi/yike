...
自主搜索与前端同步接续（2026-09-13）：内部动态搜索候选 `0cc4827` 已用 Codex＋国产模型＋真实搜索完成两轮查询、两篇原文读取，38.80秒；只证明搜索→原文的技术链，旧帖不计有效线索。独立审核发现异常子进程回收与中文结果字节上限两个问题，当前修复中，[唯一实施/失败/限定证据](superpowers/plans/2026-09-12-public-search-research.md#evidence)。上方能力尚未接客户任务，不宣称已上线。按用户要求已同步完成[前端源码体验审查与接续顺序](../design/RESEARCH_EXPERIENCE_ALIGNMENT.md)：保留 R3/R4 八入口，下一功能批优先研究进度说人话、折叠技术明细，再随通用研究接已确认策略/许可/证据一起联调；语音、多轮业务理解仍需真实实现和验收。不是另建纯聊天产品，完整 Goal ACTIVE。
