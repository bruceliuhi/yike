...
|---|---|---|
| P0 / 02、03、05F/G、09B/C | Win：Windows运行时/worker/现任务入口、小红书映射及候选/执行后端 | IN_PROGRESS；XHS受控补丁、Windows读取/私有桥及账号页登录→本机profile→版本化连接已实现（仅开发配置，真实账号未验），见[本批接线](superpowers/plans/2026-09-10-win-platform-connect.md#本批交付与验证)。接续采集前账号重验、main/worker装配、终态完成/来源capability；三渠道真实采集未验 |
