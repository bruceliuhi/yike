...
发布接续：正式包HTTPS配置`5f54d4a`已独立GO；服务镜像漏带JSON合同的启动缺陷由`2d6b79b`修复，独立GO并完成一次linux/amd64构建、原CMD非root启动healthz200/无数据库readyz503，[唯一镜像证据](qa/SERVICE_IMAGE_STARTUP_20260911.md)。用户授权部署101.200.137.138，服务未部署、域名未定；本机默认SSH key存在但被拒绝。下一步恢复合法访问后只读盘点、独立部署/数据库/HTTPS，并接同版本Windows真实业务验收；不重复空配置构包，不把本机镜像存活当生产或完整Goal完成。
