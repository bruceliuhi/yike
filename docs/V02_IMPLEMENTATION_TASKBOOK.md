...
### 2026-09-23 当前认证决定与动态证据修复（`4347f62`）

- 以 `AUTHORITY.md` 的 2026-09-13 决定为准：新客户端首次短信验证成功即自助开通连续 72 小时试用，不要求 8 位试用码；同一手机号重复登录不重置试用。远端曾合入的“恢复 8 位试用码门禁”仅是并发文档/候选，不得覆盖该用户决定，也不作为本包认证契约。
- 动态候选发布现在只从同一任务的不可变 `SEARCH` journal 按原文 URL 绑定搜索 query，并校验配对回执摘要；找不到匹配时保留未知，避免把候选误标为来自某个查询。定向回归 **112 passed、28 skipped**；未部署、未宣称真实公网研究成功。
- 当前真实短信登录仍受阿里云历史 `PORT_NOT_REGISTERED` 阻断；这不是用试用码或后四位绕过的理由，须完成运营商端口报备后再做真实收码验收。
