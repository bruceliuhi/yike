...
| V02-06 真实触达通道 / 通道＋后端 | IN_PROGRESS | 已冻结来源、收件人映射、渠道能力和回执状态的严格契约；下一步核对真实平台能力与服务端连接版本 | 依赖 01 身份；真实采集对象到收件人映射、可联系性、确认后真实发送和关联回复；失败原因准确 | 契约候选 `74e8ff6`、[V02 触达协议](contracts/V02_OUTREACH_CHANNELS.md)、专项7项通过；仅 contract-only，不是平台接通或真实发送 |
| V02-07 草稿、确认与发送队列 / 后端＋前端 | IN_PROGRESS | 前端三队列、独立草稿、确认快照、持久防重及原请求核对已实现；R4补短句教练与联系准备交互；后端契约已冻结，继续接真实队列、生成/发送/回执后台 | 依赖 04/06；对象、来源版本、内容版本、渠道或连接改变使确认失效；真成功/失败/未知对账；未经确认不发送 | [R3 交互增量](qa/ui-interactions/REVIEW.md)、[R4 验收](qa/ui-r4/README.md)、契约 `74e8ff6`；默认真实通道及短句服务尚未接入，不等于已生成客户草稿或已发送 |
...
| V02-05A / CodexMac，保留原前端工作及主线交接记录 | IN_PROGRESS | 最新4eb1bb8补齐待办空间隔离、模板搜贝/执行上限及无人工记录回复入口；迟到列表不覆盖本地选择，独立限定终审通过。桌面1253 passed / 22 skipped，类型、隔离Mac构包与严格smoke通过；新包与330项输入逐字节绑定，详见[本批验收](qa/ui-reply-entry/README.md)。旧[R4](qa/ui-r4/README.md)、[20页基线](qa/ui-final-acceptance/REVIEW.md)及[状态恢复](qa/ui-state-recovery/README.md)各自保留 | Mac锁屏，新增回复选择器及当前包可见冷启动/退出/重启尚未验；继续剩余同状态视觉、原生选择器及Windows验收。真实后台、平台与计量收费按原05B～G/07C/08B/09C/E接入，TEST与未提交raw草稿不计真实能力；05A/Goal不标DONE |
