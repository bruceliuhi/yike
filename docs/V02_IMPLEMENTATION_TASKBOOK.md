...
2026-09-12离线包接收发现并修复触达4模块未入固定清单的问题，源码候选`433a479`与隔离真实导入RED→GREEN见[运行包接收修复](superpowers/plans/2026-09-10-win-portable-runtime.md#2026-09-12-mac触达来件的离线包接收修复)。旧运行包摘要不含新触达代码，不能作为本轮验收；正式包bootstrap仍未接，后续须由新源码生成触达版运行包并实际搬迁验证。此为Mac触达来件的必要接收补丁，不冒称Win已ACK或运行；完整Goal继续。
