...
动态公开研究/交付架构接续（2026-09-12）：用户最新确认后台优先复用Codex开源Harness、接国产商业模型API，允许使用本机已有国产模型key作有界兼容性验证；客户只登录意客，无须Codex账号/安装/key。全网发现不以固定网站目录为边界，采用通用研究Skill＋独立搜索/原文工具＋授权平台深读。完整V02 Goal不重建、不缩小，继续到真实交付上线。[设计](superpowers/specs/2026-09-12-open-web-research-design.md)、[实施与证据](superpowers/plans/2026-09-12-open-web-research.md)。当前在开发通用读器与工具协议，并验证国产模型多轮工具兼容；本条不是运行/部署成功记录。UI目标式输入与实际研究进度随真实能力接通，不做纯动画宣称全网搜索。
