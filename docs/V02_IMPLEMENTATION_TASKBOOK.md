...
1. **部署关（进行中）**：a91ea42独立管理配置与残留socket恢复，9项定向及独立复核通过；服务镜像构建32750已exit0，研究镜像与服务器真实隔离/超时/异常恢复/旧任务不重启待验。客户生产研究尚未启用。
2. **客户端闭环关（待部署）**：新版Mac用户再次登录后已进入工作台，本次未再出现简报时效错误；画像v1加载成功。真实搜索→原文与候选→草稿复制→反馈仍待实际客户端验证，不以旧包或后台样例代替。
