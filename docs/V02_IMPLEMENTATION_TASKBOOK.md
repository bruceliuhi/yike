...
发布接续：正式包已改为构包时绑定公开HTTPS origin，缺地址make失败、客户启动不再依赖终端环境；`5f54d4a`独立GO，[唯一证据](superpowers/plans/2026-09-11-release-service-configuration.md#实施与验证)。用户授权部署101.200.137.138，服务未部署、域名未定；本机默认SSH key存在但被拒绝。下一步恢复合法服务器访问后只读盘点，意客独立部署并接同版本Windows包和真实业务验收；不重复空配置构包、不覆盖其他项目，完整Goal不关闭。
