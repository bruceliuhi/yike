...
|---|---|---|---|
| 先让客户自己建任务 | 01D 正常登录；04A/B 资料→确认画像/策略/预算的最小服务；完成在途01C所需执行身份，不继续扩高级授权功能 | 05B/C/D 按已ACK契约接入原R3页面；09B/D生命周期与隔离存储 | 客户不拿CLI token，不手填内部ID；形成真实、持久化、可修改和确认的任务输入 |
...
| 同批完成行动和亮点 | 06A及07A先核验实际路径；06B/07B确认发送；08A回复与跟进；04B固定“多找类似”和独立短草稿 | 07C/08B接回执/回复；05C/E展示扩展原因与新增；09C最小可安装包 | 真实来源对象可联系、确认后发送、回复回原机会；03B至少两批变化/防重可见；不等所有来源完工才验证收发 |
...
|---|---|---|---|
| V02-04A / CodexWin（用户授权新增分担），Mac独立接收 | NOT_STARTED（已认领；当前为契约/文件边界细化，非实现完成） | 基线 `5022b36`；拟新增独立 `pilot/business_profiles.py`、`pilot/materials.py`、`pilot/business_api.py` 及专项测试，复用现有PostgreSQL/身份及P03/P04；共享迁移编号、`pilot/ui_api.py`和Profile页面接入须先划定范围 | 先让一个真实业务保存/确认画像、资料与引用授权可用，解锁搜索及联系；随后补独立多业务管理和资料其余操作，复用`UI_MATERIALS_CONTRACT.md`。必要撤销、租户/版本/幂等随首次引用交付，真实PG、API和05B联调后独立审核，不能将本机草稿或假AI当后台。未收到Mac对共享入口的ACK，不抢改在途文件 |
