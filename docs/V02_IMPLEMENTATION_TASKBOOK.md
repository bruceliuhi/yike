...
|---|---|---|
| P0 / 02、03、05F/G、09B/C | Win：Windows运行时路径/私有目录/进程清理、真实worker、上传批次恢复和既有任务入口；Mac：小红书映射及候选/执行后端 | IN_PROGRESS；上传674eafc、进程清理e520850、原批次恢复0263d49、自动安装2560a40已交付；后台scope/worker现通过真实HTTP/PG原批次恢复组合与独立审核，见[交接](handoffs/V1_WIN_FUNCTION_OWNERSHIP_20260909.md#分工调整)。Win继续实际driver/main/bootstrap；Mac终态完成/来源capability及在途映射待收口，三渠道真实采集未验，不并改后端 |
| P0 / 06、07、08 | Mac：小红书真实发送可行性、发送/回执与回复存储修复；Win：现确认/跟进页面真实接入 | IN_PROGRESS；先验证真实主渠道，不以合同/草稿当发送。人工核验回复登记允许，自动回复后排 |
| P1 / 04、05B/C/E/G、07 | Win：最小业务画像/关键词、列表原文、实际多找类似；Mac：真实判断/短句生成服务 | IN_PROGRESS；复用已交付证据/策略模块，补真实消费，不扩高级评分/管理 |
| P1 / 09C、10 | Win冻结一个可安装候选；双方一次整批独立审核及真实链路验收 | NOT_STARTED（本精简版整体验收）；现1a47178包仅历史入口证据，非本阶段完成 |
