...
|---|---|---|
| P0 / 02、03、05F/G、09B/C | Win：Windows运行时路径/私有目录/进程清理、真实worker、上传批次恢复和既有任务入口；Mac：小红书映射及候选/执行后端 | IN_PROGRESS；上传工程片674eafc及Windows进程清理工程片已交付，证据见[交接](handoffs/V1_WIN_FUNCTION_OWNERSHIP_20260909.md#分工调整)；运行时路径/私有目录、真实worker与三渠道采集未验。Mac保留在途映射，Win不并改 |
