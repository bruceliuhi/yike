...
完整V0.2剩余真代码缺口已与外部验收分开：普通模型判断的provider用量留存、跨轮来源游标、作者更新专属变化事件、多源研究及覆盖补查仍待实现；已有搜索／普通监控／复核／单通道收发应补真实客户端验收，不重新造一套。当前已安装5fb7d65尚未包含fb28f4b客户端ASSESS等待修复，下次计划内候选必须包含；登录／连接可先验，不以旧包宣称判断修复已交付。
