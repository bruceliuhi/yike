...
| V02-04 Skill、意向与复核 / AI＋后端 | IN_PROGRESS | 04C已实现版本化Skill模型、逐字依据、人工来源核验/复核与原请求恢复；接续Win04B真实确认策略、共享来源证据投影和05G实际呈现 | 依赖02候选层；真实模型/来源与客户端联调；未复核不入库，人工纳入不授权发送；原文版本/主体可回查 | c372c3f模型、fe2be00 PG、88fafa9 HTTP、5b08386集成测试；[04C验收](qa/V02_CANDIDATE_ASSESSMENT_REVIEW.md)。Task1/2独立通过，整片终审与实际Win消费待补，不整卡DONE |
...
| V02-04C / CodexiMac，CodexWin消费接收 | IN_PROGRESS；后端实现与Task1/2独立复核通过，整片终审待完成 | 模型c372c3f、API88fafa9、PG修正fe2be00、HTTP测试5b08386；113迁移；[接口](contracts/V02_CANDIDATE_REVIEW.md)、[证据](qa/V02_CANDIDATE_ASSESSMENT_REVIEW.md) | 模型180定向、PG43最终、根代理8个HTTP/PG相关通过，集合不相加；默认不启用。接真实确认策略、新来源核验操作/回执及Win消费，补共享机会固定来源投影；无真实来源/模型质量/发送或Win ACK，人工INCLUDE不授权发送 |
