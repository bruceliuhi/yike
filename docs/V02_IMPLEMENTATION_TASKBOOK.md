...
范围恢复时同步基线为 `origin/main@03f2705`。Win 暂停分支 `codex/win-collection-flow@96bb371` 的回复持久层修复现已由Mac限定接收为 `4c683ac`，见[本批QA](qa/V02_REPLY_PERSISTENCE_CONTRACT.md#mac-接收-win-回复修复2026-09-10)；同分支离线运行包仍未接收。Mac 原 `codex/launch-resume` 工作树的运行包在途改动原样保留。Mac负责接收审核及后端闭环；Windows运行时、客户端/平台入口和业务画像/策略的既有Win文件边界保留，接续前核对来件，不双写。独立审核由非作者执行；真实账号挑战及发送的对象/内容/渠道确认仍保留，不因恢复开发而扩大外部操作授权。
