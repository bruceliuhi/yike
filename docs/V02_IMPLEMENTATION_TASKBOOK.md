...
**2026-09-10 用户已明确恢复上一批未完成工作并要求完成后提交 Gitee。** 离线运行包两项失败已关闭，同包实际搬迁复验通过；回复持久层修复完成独立审核。精确版本、测试复用及本轮 PG 环境失败见[本批记录](superpowers/plans/2026-09-10-win-portable-runtime.md#本批交付与验证2026-09-10-恢复后)；[暂停交接](handoffs/WIN_PAUSED_20260910.md)保留历史，不再表示当前仍须等待恢复。本批未完成客户 bootstrap、真实收发及精简版整体交付，父卡和产品 Goal 不标完成。

## 当前执行范围：精简可验收版优先（2026-09-10）

用户已批准 `LEAN_ACCEPTANCE_FIRST`；具体验收只以[产品计划第1.0节](V02_COMMERCIAL_RELEASE_PLAN.md#10-当前优先精简可验收版2026-09-10)为准。下面历史完整父卡继续保留，后排不等于完成；旧“全部V1.0完成才交付”不再是本阶段门槛。

**2026-09-10 用户确认 Mac 已停止开发：CodexWin 接管本阶段全部剩余主线，包括原 Mac 的小红书映射、执行/候选后端、判断/短句、确认发送和回复。** 以下历史 Mac 认领/待 ACK 只保留来件证据，不再构成等待条件；Win先核对已提交实现再续做，独立审核仍由非作者执行。真实账号登录/挑战和发送前对象内容渠道批准仍需用户，不因独立推进扩大外部操作授权。

| 顺序 / 原卡 | 本阶段工作与实际主责 | 当前状态 / 下一动作 |
|---|---|---|
| P0 / 02、03、05F/G、09B/C | Win：Windows运行时/worker/现任务入口、小红书映射及候选/执行后端 | IN_PROGRESS；同浏览器账号重验、任务入口→worker→原文候选上传→签名 FINISH、原批次恢复及状态页已实现，仅开发配置开放，见[本批接线与验证边界](superpowers/plans/2026-09-10-win-collection-flow.md#本批交付与验证)。接续客户 bootstrap 与真实 XHS 批次/Windows 生命周期；三渠道真实采集未验 |
| P0 / 06、07、08 | Win：小红书真实发送可行性、发送/回执与回复存储修复、现确认/跟进页面接入 | IN_PROGRESS；回复持久层修复已审核交付，见本批记录；真实发送、回执和页面全链仍待验证，不以合同/草稿当发送。人工核验回复登记允许，自动回复后排 |
...
| P2 / 10D | 真实案例：100候选、30个对象批准联系、指标及搜索/画像/规则修正 | NOT_STARTED；一期通过后，以用户业务账号和明确联系授权执行 |

`DEFERRED_AFTER_LEAN`：其余平台、持续监控/高级调度、复杂CRM/管理/统计、计费、多租户管理、自动回复、自动更新/完整升级矩阵及完整跨行业商业验证。保留原卡及源码，不移除已有隔离、防重、资源上限和恢复。短评论/私信草稿、原文证据、多找类似不延期。当前认领和文件边界沿用[Win/Mac交接](handoffs/V1_WIN_FUNCTION_OWNERSHIP_20260909.md#分工调整)；Mac未实际确认的新接续不冒称ACK。

## 历史批次与完整范围状态（保留证据，不作为一期全部前置）
