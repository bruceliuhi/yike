...
动态公开研究/交付架构接续（2026-09-12）：通用匿名HTTPS读器＋官方MCP工具代码`7cb00f6`已获独立整批GO（修复含两次delta，现存0个P0/P1/P2）；公开测试页实际读取成功，Codex0.153.4＋豆包经临时薄协议适配完成真实工具往返。适配器仅仓外诊断，**自主搜索/研究控制器及客户接线仍未实现**；不等于可部署Worker或有效商机。用户确认通用研究扩范围，已有小红书/抖音等连接器与授权浏览器继续承担登录、评论深读、监测及成本优化；追加允许Qwen对照，目前未定位凭据所以未测。[唯一设计](superpowers/specs/2026-09-12-open-web-research-design.md)、[实施/失败/限定证据](superpowers/plans/2026-09-12-open-web-research.md#evidence)。下一步生产化模型协议适配、独立搜索工具和Skill研究，再接现有任务/预算/证据/真实进度。客户只登录意客，不需要Codex账号或key；完整V02 Goal继续ACTIVE，未部署、不重新构包。
