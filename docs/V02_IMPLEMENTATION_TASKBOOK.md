...
## 2026-09-26 Lead Radar 产品整合

统一归属和内部工具边界见[整合状态](INTEGRATION_STATUS.md)。共享规划已接真实云端研究与原客户端预览；原登录、租户、PG、签名任务、候选复核和搜贝链路保留。首页样例收成单入口，任务/监控直接进入对应线索，原文及运行明细按需展开。简报补充本人范围内的证据缺口与下一步。

定向验证已取得：原会话HTTP预览与签名任务的计划一致、不新增任务/用量账及客户上下文隔离测试38通过；简报真实受限PG及规则26通过；规划/上下文80通过；Lead Radar/Codex worker158通过；最终前端协议、预览、工作台、任务、监控和原文UI共108通过；独立wheel安装验证1通过，类型检查通过。各批存在重叠，不累加为总测试数。

浏览器已验证真实任务向导内的计划展开、关键词修改后旧结果立即清除及新结果更新、窄窗口无横向溢出、原导航可返回精简工作台；测试页面明确标识TEST内存夹具，不能充当真实搜索证据。搜索计划的规则与夹具生成结果一致，桌面生产构建不包含视觉夹具入口。

Mac arm64验收包基于 `b943710` 一次构建完成：包结构、包内main/preload/renderer启动、固定IPC、沙箱、CSV/备份实际临时文件写入及取消、未保存退出确认均通过；相关冒烟策略13项通过，secret_scan干净。ZIP SHA-256为 `c12a592993415566a5fa944ce001377830968e030c780d4b37e575fb9cf9c56a`，ASAR为 `9945aa242840500d76d765f7afa819cfc8b1622e12a397406389e3f22d4ab986`。该包仍为ad hoc签名、未公证的本地验收包，配置的服务地址为 `https://yike.tuokexing.net`；新增服务能力需部署同版后使用。本次一次性PG测试容器已停止并清理。

本批未执行真实平台搜索/发送。公网健康检查仍回报旧revision `5330c81046b636b6cad65589581374e321a277c5`，本次服务器SSH连接超时，仓库未提供可用替代连接参数，尚未部署本批代码。不能把本地集成通过当作生产原文读取及商机质量已解决。

## 当前权威状态（2026-09-23，候选运行代码基线 `1fcef0b`）
...
- Gitee `yike-ai2026/main`、GitHub `main` 与本地 `main` 已同步包含候选运行代码基线 `1fcef0b176c99110223798322b77b386aa50f4bd`。
- 当前本机发布检查为 `local=PASS / external=NOT_VERIFIED / overall=HOLD`；Lead Radar 当前主线复跑 `89 passed`，macOS arm64 包结构与包内冒烟通过，当前包证据见 [`latest-main-package-1fcef0b.json`](qa/ui-candidate-mac/latest-main-package-1fcef0b.json)。
...
- 双语计划测试 **2 项通过**，完整 Lead Radar 回归已覆盖该路径；多语言证据展示、界面标签和导出仍未宣称完成。
- 该切片只扩大计划编排能力，不改变来源权利、真实平台授权或 SEARCH→READ→证据门禁；未配置 provider 时仍保持阻断。

### 2026-09-23 来源权利门禁与完整回归收口（`b19f26b`）
...
### 2026-09-23 公网过渡版本记录（`5330c81`，不作为最终发布）

- 公网曾回报过 `5330c81046b636b6cad65589581374e321a277c5`，健康/就绪为 200，但该版本把首次试用改成仅短信验证，违反已确认的“手机号 + 短信验证码 + 8 位试用码”流程；不能作为客户交付版本。
- 服务器加固、HTTPS 健康检查和回滚坐标只作为环境证据保留；真实短信端口、平台连接、SEARCH→READ→候选→证据及客户价值仍未验收，整体继续 **HOLD**。

### 2026-09-23 公网过渡版本记录（`4a7aed0`，不作为最终发布）

- 公网当前回报过 `4a7aed06cac6126d5be8f606de69a4953bf4ffae`，健康/就绪为 200，但该版本把首次试用改成仅短信验证，违反已确认的“手机号 + 短信验证码 + 8 位试用码”流程；不能作为客户交付版本。
- 服务器镜像、容器加固和 HTTPS 健康检查记录保留作环境证据；真实短信端口、平台连接、SEARCH→READ→候选→证据及客户价值仍未验收，整体继续 **HOLD**。
...
- 该版本记录中的 Mac 包哈希 `50c2be…` / `1506ca…` 与过渡认证逻辑绑定，不能作为当前试用码流程的交付包；当前可交付 Mac 包仍以 `latest-main-package-0a17cf7.json` 为准。
...
- 登录页定向回归 **17 项通过**，试用/运营相关 Python 回归 **5 项通过、17 项跳过**，类型检查通过。Mac 包字节仍对应前一候选代码 `0a17cf7`，本轮认证最终文件与该包代码一致；正式生产切换仍需重新部署并核对运行 revision。
- 远端线上服务当前回报 revision `2f75d91df21cf66a992d29d321d53125c421a0b7`，本地主线为 `436991c`；健康/就绪为 200，但线上仍停在取消试用码的过渡提交，同源版本、真实短信收码和客户 UAT 仍未完成，正式上线保持 **HOLD**。
...
- 本轮新增一次性 PostgreSQL 16 隔离验证：空库迁移返回 `{"status":"migrated"}`，受限非超级用户授权/RLS 套件 **2 项通过、0 失败**；容器已清理。该证据只加强本地迁移与最小权限信心，不替代生产数据库、备份恢复和回滚验收。
...
- 当前线上能力回读中的 `access_login.available=false`：临时访问码旁路未接线，调用会返回 `capability_unavailable`（501）。不得使用手机号后四位、旧试用码、注入会话或手工改库绕过短信认证。
- 最近真实登录失败仍对应阿里云短信 `PORT_NOT_REGISTERED`；当前尚无新的可消费短信挑战。必须先完成运营商端口实名报备，再做一次真实收码/登录验收；在此之前总体保持 **HOLD**。
...
- 按该提交以 `YIKE_RELEASE_SERVICE_URL=https://yike.tuokexing.net` 重新构建 macOS arm64 包：ZIP SHA-256 为 `3d710ebf353a705d147ea169bab4efe56fe7815c6e5f48fcec2403191e06df56`，ASAR SHA-256 为 `71f85d7f45a36a74cdd6b1656fd6b92d8689406ac83cbe7fa0db8005c3c13726`；包结构校验和受限包内冒烟均通过。机器可读记录见 [`latest-main-package-866ef86.json`](qa/ui-candidate-mac/latest-main-package-866ef86.json)。
...
- 本轮本机验证：Lead Radar **34 项通过**；发布候选检查 `local=PASS / external=NOT_VERIFIED / overall=HOLD`。Windows 专属测试在 macOS 上因 Windows 路径/运行环境失败，按当前范围不作为本轮产品判断；正式上线仍保留 Windows 交付门禁。
...
- 最终运行容器的 `Config.Image` 已核对为上述 registry digest，而非可漂移标签；当前回滚副本为停止状态的 `yike-customer:bc38fa0-candidate`，此前两个历史回滚副本仍保留。
...
- 原文重开增加机会级幂等回放：相同机会和 `Idempotency-Key` 直接返回既有证据与计量结果，不重复访问页面、不重复扣费、不追加证据。
...
- 真实短信挑战显示供应商受理、未消费、未过期；在生产受限 `yike_app` 权限下以回滚事务验证自助注册函数成功，未修改业务数据。此前阿里云 QuerySendDetails 两次返回 `PORT_NOT_REGISTERED`，故当前失败仍归类为短信实际投递/用户拿到的验证码问题，不通过后四位或弱化认证绕过。
- 线上能力回读仍为 `platform_connections=false`、`task_execution=false`；小红书、抖音在新客户端显示“待连接”，尚不能写成真实平台采集或任务链路已上线。生产容器 revision 仍须与最终主干重新绑定并完成真实登录、研究和平台验收。

### 2026-09-23 主线收口复核（当前主干 `d6f95a8`；功能基线 `3337ca6`）

- 功能实现源码截至 Lead Radar 空态与局部加载韧性修复固定于 `336d70955c2f3942b4d2bce70302a7294095234c`；随后仅追加发行记录。功能基线包含授权搜索 API 执行门禁、来源 proof artifact 门禁、任务运行事件、候选校准批次、审计/用量面板、索引结果复核入口、独立管理员备份恢复、明确来源空态和局部加载失败隔离，以及桌面端连接能力状态提示。推送前仍以 `git rev-parse HEAD` 做最终版本核对。
- 桌面端在 Node 24.19.0 下完成类型检查、生产 renderer 构建；稳定单线程全量验收为 **268 个测试文件通过、26 个跳过，4563 个测试通过、55 个跳过**。Lead Radar 后端当前为 **25 项测试通过**，包含索引结果 proof 绑定、审计用量、证据绑定的跟进草稿与人工批准门禁。
- 健康接口现在返回 `X-Yike-Release-Revision`，`scripts/cp06_probe.sh` 可用同一最终 SHA 核对部署版本；缺少版本头或版本不匹配不能以健康 200 代替发布验收。
