...
`ResearchStrategyStore.resolve(cursor, claims, profile_version_id, strategy_version_id)`供[执行合同](V02_EXECUTION_RUNTIME.md)及04C使用，返回真实`ConfirmedExecutionStrategy`。它校验当前用户的当前已确认版本、画像仍CONFIRMED且完整payload摘要与prepare一致、不可变快照及摘要重算一致；不读取客户端自报已批准结果。
