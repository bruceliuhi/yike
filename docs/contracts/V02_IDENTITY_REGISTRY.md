...
状态：代码候选，等待独立复审；不是 V02-01 完成或真实平台连接证明。
...
正常客户激活/登录、服务端会话撤销、设备凭据绑定、真实平台扫码/账号核验、连接凭据更换后的旧执行失效、调度和游标 fencing、Windows sidecar 均未由本候选实现。现有 token 退出仍只清除客户端 Cookie，不能宣传为所有访问令牌已撤销。
