...
- 管理员发布作业先执行既有 `yike-pilot-migrate`，再对既有应用角色执行 `deploy/grant_session_revocations.sql` 的身份登记最小授权，之后才加载新版应用。该文件名为兼容既有 105 发布命令保留，现同时补齐 104 设备/连接/事件表的显式最小权限；105 会话撤销表本身仍仅授予 SELECT/INSERT，不授予 UPDATE/DELETE。仅迁移不代表应用可访问新表；精确命令见[运行手册](../CUSTOMER_PILOT_RUNBOOK.md)。新增迁移为 `105_v02_session_revocation.sql`，不修改已合并的历史迁移；Web 不获迁移权限或管理员 DSN。
- 该授权脚本不创建角色、不使用全表或默认授权；只改变上述四张身份登记表的显式权限。应用仍只需对 pilot_users 的既有 SELECT，不因普通鉴权获得用户表 UPDATE。脚本必须在受信管理员连接执行，授权前检查目标存在且不是超级用户、BYPASSRLS、CREATEROLE 或任一目标表 owner。
