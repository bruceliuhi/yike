...
更新：2026-09-09。状态：`PENDING_RECEIVER_ACK`。本文件记录真实开发进度，不是 CodexWin 已接收、接口依赖已就绪或功能已上线的证明。
...
- 组合候选 `2f0626c` 合入测试时钟修复后，整合复核发现 104 三张新表的应用角色授权遗漏，结论 `REQUEST_CHANGES`。正在修复并重新锁定接收版本；单项 PASS 不能替代组合验收。
...
1. `git fetch origin`，在干净隔离审查工作区锁定候选提交，保留其他任务的修改。
2. 阅读上述契约、`docs/qa/V02-01A_REVIEW.md`、`V02-01B_REVIEW.md` 和组合复审记录，确认客户端可正确呈现字段、状态与限制。
3. 准备一次性 PostgreSQL，管理员/应用角色分离。未发布的 104 旧候选 checksum 不得覆盖；新空库按候选运行手册执行迁移及最小权限升级。真实数据库凭据不提交 Git。
...
5. 下表填写接收 SHA、日期/环境、复现命令与结果、`ACK` 或 `RETURNED` 及问题。只接受具体版本，不填写笼统“已看过”。

## 接收记录

| 接收方 | 接收 SHA | 时间/环境 | 复现结果 | 结论 |
|---|---|---|---|---|
| CodexWin | 未收到 | 未提供 | 未提供 | `PENDING_RECEIVER_ACK` |

本机独立 Agent 不替代该签收；不假定两端共享对话或自动唤醒。无依赖的适配器/Windows 工作可继续，依赖接口在明确 ACK 前保持未就绪。

## 继续开发

CodexiMac 继续正常手机号登录/激活与设备认证。P01 保持手机号＋验证码＋可选试用码，不改成密码登录或要求客户用 CLI 取 token。短信供应商、签名/模板、生产凭据与真实手机尚未验收，不能打开 sms_login。验证码不能进入业务数据库或日志；身份映射、一次性消费、限流、恢复和客户端白名单按契约继续开发。
