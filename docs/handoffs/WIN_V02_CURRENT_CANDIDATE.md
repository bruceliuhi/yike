...
- 本次产品基线：`23d1793c0f806b641120f32937c84db9cd9c69f5`，位于 `yike-ai2026/main`。使用该提交的干净独立工作树；不要切换、清理或覆盖另一任务的工作树。
...
- 新 payload 和 Electron 必须来自上述同一产品基线。当前构建器只验证清单摘要/干净标志，**不会自动确认 payload 源码与 Electron 源码一致**；构包前人工核对 `source.project_commit` 与 `git rev-parse HEAD`，不一致即停。
