...
新代码候选接续（2026-09-12）：`ab9c6fe`实现B站内容/作者links受控采集，非作者整批GO；当前安装版`1f597f6`不含此功能，不追认也不立即重复构包。[实现、首次截断修复与有限证据](../superpowers/plans/2026-09-12-bili-link-execution.md#evidence)。下次计划内同源候选须同时包含新0004/锁摘要、portable中native_collection_links模块、host/TS/服务端新mode；先在独占runtime按锁准备，不就地修改现有profile/runtime。仅在真实登录/B站授权后按一条视频和一页作者的有界路径验正文+评论、归因/重复、停止及失败状态；未授权不读账号，不发外部消息。

最新Win ACK（2026-09-12）：`1f597f6` 已推main并实际构包/覆盖升级/启动READY，包含主进程75秒与新增页面90秒ASSESS修复，62项定向＋tsc及独立GO。[唯一产物/验证/未验记录](../qa/WINDOWS_SYNC_20260912.md#已构建并覆盖升级1f597f6)。customer继续5fb7d65（服务端无差量），不重复部署/旧安装全套/实网模型。下一步用户本人登录与平台认证后验真实业务；未签名、未外发、未完成客户验收。下方“修复尚未入包”为历史状态。
