...
- `yike-ai2026/main` 是唯一产品主线；功能任务从最新 `origin/main` 建 `codex/<主题>` 分支。本次用户明确要求的文档/目标调整由 CodexWin 直接提交 main；不改变后续功能开发流程。
...
- [ ] V02-06A/07A 的主体映射与操作契约 ACK 后，实现 V02-06B 通道适配和 V02-07B 确认队列；模块测试不互等真实整链，真实收发交 V02-10B 收口。
- [ ] V02-07C 接发送确认和未知恢复；V02-08A/B/C 分别交付回复/跟进 API、工作台和价值/成本查询。页面不与 Mac 已在途文件并发编辑。
...
- [ ] V02-09C/E 分别验证安装/卸载/授权/备份和更新/回退；用户手动实机步骤未回传前不标通过。
