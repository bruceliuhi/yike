...
- [x] 写测试并确认RED：严格输入/密钥、摘要无敏感值、迁移幂等与最小角色、无上下文读取为空、应用不能绑定、号码不能串用户、冷却/配额持久化、5次失败持久化、超时/旧回执、两线程仅一次消费。使用合成号码，真实隔离PG。
- [x] 最小实现并跑GREEN：`uv run --frozen pytest -q tests/test_phone_auth.py tests/test_phone_auth_postgres.py`；33通过/0跳过。
- [x] `9556fab`提交并由phone_auth_task_review独立通过；无真实短信支持结论。

### Task 2: P01 HTTP与桌面服务接入

**Files:** Create `pilot/phone_api.py`, `tests/test_phone_api.py`, `tests/test_phone_login_postgres.py`, `desktop/tests/ui/phoneClient.test.ts`; Modify `pilot/ui_api.py`, `pilot/web.py`, `desktop/src/shared/contracts.ts`, `desktop/src/main/servicePolicy.ts`, `desktop/src/renderer/services/client.ts`, `desktop/tests/servicePolicy.test.ts`. No page-layout edits. 集成时仅为已复现失败补充`desktop/tests/ui/task-actions.test.tsx`的异步同步修复，不改生产任务逻辑。
...
- [x] 写RED：默认关闭；HTTPS/Origin/严格body；错误码、未知号码、超时不重发、试用码不可用；客户端映射与恶意字段拒绝。补真实PG请求→供应商调用→消费→session读取→退出拒绝旧cookie集成验证。
- [x] `f36ec97`目标Python/desktop服务策略和登录相关测试、typecheck/build、会话及真实PG回归通过。测试短信替身仅验证代码调用，不作为供应商实测；最终集成回归见交接。
- [x] 更新01D交接与唯一任务书，列清真实供应商、签名/模板、凭据、手机收码、激活/到期和Windows ACK缺口；`99bb62d`及修正`fe78c9e`独立整体/增量审核通过，不升级整卡DONE。main推送事实以最终Git核验记录为准。
