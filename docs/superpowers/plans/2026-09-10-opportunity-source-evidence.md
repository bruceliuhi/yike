...
**Persistence:** table `pilot_opportunity_evidence` with `(tenant_id TEXT, opportunity_id TEXT)` primary key, same pair FK to pilot_opportunities ON DELETE CASCADE, internal `included_by_user_id TEXT NOT NULL / include_request_id TEXT NOT NULL`, `payload JSONB NOT NULL`, `payload_sha256 TEXT` lowercase64hex. Enforce payload is object/schema/version/opportunity binding; enable and force RLS: SELECT by tenant, INSERT by tenant and included_by_user_id=current user. INSERT trigger checks the same owner's existing private review/request points to this opportunity and is INCLUDE/SUCCEEDED/IMPORTED, with payload assessment ID/profile version/source version matching that request. This is a local write-time check under existing owner RLS, not SECURITY DEFINER or a shared history read; no private-history FK to erase or block retaining the approved copy when private history is cleaned. UPDATE trigger always rejects evidence changes; no runtime UPDATE/DELETE/TRUNCATE or table ownership. Explicit repeatable grant script accepts only existing non-superuser/non-bypass/non-createrole/non-owner application role, revokes previous rights on this new table and grants SELECT,INSERT only. Register115 once in existing migration list.

**Insertion seam:** after actual import and existing private request/review insertion, only when created, fetch the exact observation using the existing cursor, construct the allowlisted payload, and insert before final `_active` with the same cursor. No extra transaction or independent commit; any failure also rolls back opportunity/source changes/review/receipt. No ON CONFLICT overwrite or silent evidence failure.
