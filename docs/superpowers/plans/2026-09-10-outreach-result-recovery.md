...
代码b8f3e3f的独立整批审核发现唯一P2：目录同步失败后保留的完整文件，被后续read/幂等put未补目录同步就当作持久结果返回。原作者按差量补齐成功返回路径，修复及最终审核待记录。该结论来自静态代码路径，审核方未成功运行的内存加载尝试不算验证证据。

2026-09-11收口：修复提交`97319a044f3e86e58b5d1091a87b84f83ad07ae3`覆盖read、幂等put、并发命中及新建成功返回路径；目录打开/同步/关闭失败统一失败关闭。新增目录故障用例RED 1失败/1通过，修后outbox专项10/10通过；其余既有证据复用，未重复全量测试或构包。独立审核仅检查`b8f3e3f..97319a0`差量，关闭原P2，结论GO（代码候选），无新增阻断；不涵盖真实driver/main/UI、Windows实机或上线验收。父卡与Goal继续IN_PROGRESS。
