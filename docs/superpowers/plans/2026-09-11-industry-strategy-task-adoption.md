...
基线 `cf1f912`；后端 `ac4f94c`、客户端 `95b184d`；独立整批审核 GO，无可操作发现。Task1/Task2实施已完成，以上步骤为计划原文；不关闭完整V02或父卡。

- 入口：新建采集/监控任务→原搜索建议弹窗单独“采用任务策略”→表单编辑/移除/按当前画像确认→最终策略快照。词项采用独立，已有人工策略不被覆盖。
- 后端新增测试RED为2失败/5通过（旧协议不接受字段），修复后7通过/0.84秒；其中受限本地PG验证prepare→confirm→resolve保留新配置、新修订阻止旧resolve、历史回执不改。旧字节/摘要精确选中3通过/0.15秒；py_compile/diff检查通过。合成PG容器已清理，无迁移/授权变化。
- 客户端初始4项RED（草稿丢字段、旧画像不阻止、快照无展示、缺采用入口），接通后4通过；完整本批5项新增与2项受影响旧测试共7通过/2.55秒，TypeScript检查通过。未重复整套、未构包。
- 已确认的是可编辑条件及版本绑定，不是候选模型已采用规则；界面明确“尚未用于自动评分或内容筛选”。后续从已有服务端候选快照读取industryStrategy传入受控模型，不拼入原文证据。真实平台/模型质量、Windows、生产和跨行业UAT仍未验收。
