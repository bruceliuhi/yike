...
本批最终源码 `73be8fc096b0a9e940d23281811fae9710c909df`，独立非实现作者整批审核及差量复审 **GO**。前端 `81801a7`、后端 `64c0d8d`、普通客户端联验 `af58975`/`15cfa89`；上方测试清单保留为完整边界清单，以下为本轮实际证据，不把未单独执行的矩阵项写成已验。

- **可用路径：** READY字段采用→待保存引用→画像/引用同事务保存→重开/共享最小资格继承；手改/明确改为人工内容保留文字，解除最后一项显式提交空引用。修改/撤销/移除资料使资格失效；影响确认绑定引用快照，出现新引用须重新确认。
- **首次审核 NO-GO：** 历史 READY 在撤销后可重新引用；旧成功搜索建议仍标当前；旧候选 AI 评估仍可纳入且缺派发前重验。`73be8fc` 一次修复波次关闭三项：核对真正最新资料、历史建议当前资格失效、候选当前投影/纳入/派发前检查。保留旧成功事实、人工核验与无关采集，不用取消整个画像替代引用管理。
- **前端限定证据：** 新 adapter 3项及 UI 3项通过（最终合跑6项/2.67秒）；既有画像资料入口3项通过，变化的采用回调另1项通过；集合有重叠，不累加。TypeScript通过，没有构包。初始新功能用例失败；UI测试最初误找已确认按钮，改为检验真实保存阻断；测试配置首轮路径错误没有执行测试，未算失败产品证据。
- **普通客户端联验：** `tests/test_profile_reference_client_postgres.py` 驱动实际Node→回环HTTP→受限PG，**1 passed / 0 skipped / 2.57秒**，内部Node也是1项实际运行。保存/确认/预览→新增继承→旧影响token被拒→重新确认撤销→预览拒绝→原文字/状态可读→显式空引用另存人工稿→退出后拒绝。首次仅测试缺必需AbortSignal/progress参数，修正测试，不放宽产品适配器。
- **后端定向与修复证据：** 初批资料引用/原资料状态14项通过；兼容检查曾执行225项（未再重复）。最终修复仅 `tests/test_material_profile_references.py` **10 passed / 3.43秒**，以及 `tests/test_candidate_review_postgres.py` 两条新增撤销反例 **2 passed / 2.09秒**，覆盖旧 READY 经 revoke/save/remove 均拒绝、成功/待处理建议 get/replay/prepare/finish、旧评估纳入/当前投影及预留后派发前撤销不调用模型。`py_compile`/`git diff --check`通过。独立复审直接复用最终字节证据，不重跑全套。
- **证据边界：** HTTP联验绑定修复前接线，修复后受影响撤销路径以上述12项差量证据为准；未因相同前端/传输再次跑整链。仅合成资料/模型结果及专属测试数据库，未执行真实外部模型、平台动作、Windows包、生产或跨行业UAT。

本批只建立资料→画像引用，不虚构资料→联系草稿引用；后者随实际外发引用路径接入。Windows固定候选 `23d1793` 不追认包含本批源码；若接收本批产品能力，须重新明确候选版本并按实际变更验收。完整V0.2 Goal继续，不能因本批GO标上线。
