...
- [ ] 先追加失败断言：完成序列的 `usage.resourceCloseout == {state: 'RECORDED', overduePermits: 0}`；旧代码缺字段应失败。
- [ ] 在 _dto 的读取事务先设置 `REPEATABLE READ READ ONLY`；资源分组读取同时计数 `status='ISSUED' AND deadline_at<=statement_timestamp()`。在已有阶段计算后按设计优先级派生状态，旧原始计数不改。
- [ ] TS 增加可选严格对象及跨字段校验：overduePermits<=pending；RECORDED 必须终态且无 pending/unknown；UNCERTAIN 不允许继续动作；新字段缺失保留旧服务兼容。
- [ ] 页面展示四种状态、超期数及许可/结果计数；缺字段明确服务尚未提供，不声称已结算。
