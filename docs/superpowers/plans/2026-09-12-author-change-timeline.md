...
- [x] 根整合/定向交叉验证，固定 `4fd29dc` 独立差量GO，无剩余P1/P2；代码与文档正常合main，不重复构包/部署。
- [x] 记录版本、实际通过范围、未验平台/Windows/客户闭环；跨轮来源游标继续留在完整Goal，不以作者事件子项完成关闭目标。
...
- `4fd29dc` 经非作者独立差量复审GO，原P2关闭且无新增P1/P2。复核实际brief 4000 UTF16截取与UI渲染链：首差异前最多1000字符/2000 UTF16，不会再次截掉尾部新增/替换信息；删除部分留在时间线前版本引用。合并为代码交付，不升级旧Windows包或生产验收范围。
