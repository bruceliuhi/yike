...
def _persist_records(cursor, *, tenant, user, platform, profile_version_id,
    ...
    for index, record in sorted(records, key=lambda item: source_identity(item[1], platform)):
        ...
        if version is None:
            content = record.model_dump(mode='json', include={
                'public_url','title','author_public_id','body','published_at','parent'})
            if record.source_context is not None:
                content['source_context'] = record.source_context.model_dump(mode='json')
            if record.page_metadata is not None:
                content['page_metadata'] = record.page_metadata.model_dump(mode='json')
