...
.search{max-width:680px;margin:24px 0}.search label{margin:0 0 8px}.search .actions{gap:12px}.search input{max-width:480px}
...
def users_view(rows, offset, csrf, query='', error=None):
    ...
    if len(rows) == 50:
        body += f'<a href="/ops/users?offset={offset+50}{suffix}">下一页</a>'
