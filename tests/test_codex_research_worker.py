...
def test_research_instructions_require_search_then_read(worker,tmp_path):
    ...
    assert 'buyer-owned company procurement pages' in data['instructions']
    assert 'only when the confirmed scope permits formal procurement' in data['instructions']
    assert 'rather than treating a single technical community as the default' in data['instructions']
